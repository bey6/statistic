    ...
    let process_diagnosis = (type, fileds) => {
        return diagnosis
            ...
            .map((b) => ({
                ...
                category_code: b[`${fileds}ICDCode`]?.slice(0, 3),
                suborder_code: b[`${fileds}ICDCode`]?.slice(0, 5),
                details_code: b[`${fileds}ICDCode`]?.slice(0, 6),
                extension_code: b[`${fileds}ICDCode`]?.slice(0, 7),
...
async function batchInsert(items) {
    let sql_base = 'INSERT INTO bei.table_patient (id, mrid, name, in_date, out_date, main_diagnosis_code, main_diagnosis_name,total_fee, ext_json) VALUES ',
        sql_body = ''

    items.forEach(doc => {
        sql_body += `('${uuid.v4()}', '${doc.mrid}', '${doc.patient_name}', '${moment(doc.in_datetime).format('yyyy-MM-DD hh:mm:ss')}', '${moment(doc.out_datetime).format('yyyy-MM-DD hh:mm:ss')}', '${doc.pumch.out_diagnosis[0] ? doc.pumch.out_diagnosis[0].code : ''}', '${doc.pumch.out_diagnosis[0] ? doc.pumch.out_diagnosis[0].name : ''}', ${doc.total_cost}, '${JSON.stringify(doc)}'),`
    })
    let sql = sql_base + sql_body.slice(0, sql_body.length - 1)
    client.query(sql).then(res => console.log('ok')).catch(err => console.error(err))
}
...
setInterval(async () => {
    ...
    try {
        ...
        count = ids.length
        // console.log(count)
        // let logsss = JSON.stringify(documents)
        // console.log(logsss)
        // fs.writeFileSync(moment(new Date()).format('MM-DD hh:mm:ss'), logsss)
